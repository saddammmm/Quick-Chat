...
public class SocketServer implements Runnable {
    ...
    public SocketServer(ServerFrame frame){
        ...
        ui = frame;
       db = new Database1(ui.filePath);
        
	try{  
	    server = new ServerSocket(port);
            port = server.getLocalPort();
	    ui.jTextArea1.append("Server has started @ IP : " + InetAddress.getLocalHost() + ", Port : " + server.getLocalPort());
           // ui.jTextArea1.setBackground(Color.green);
	    start(); 
        }
	catch(IOException ioe){  
            ui.jTextArea1.append("Port is busy,can't bind to port : " + port + "\n\nAttempting to bind other port\n\n"); 
          //  ui.jTextArea1.setBackground(Color.red);
            ui.RetryStart(0);
	}
    }
	...
	try{  
	    server = new ServerSocket(port);
            port = server.getLocalPort();
	    ui.jTextArea1.append("Server has successfully  started @ IP : " + InetAddress.getLocalHost() + ", Port : " + server.getLocalPort()+"");
	    start(); 
        }
	catch(IOException ioe){  
    ...
    int pos = findClient(ID);
        int pos1=clientCount-1;
        if (pos >= 0){  
            ServerThread toTerminate = clients[pos];
          //  ui.jTextArea1.append("\nRemoving client thread " + ID + " at " + pos);
            ui.jTextArea1.append("\nOne user has just signed off " );
              ui.jTextArea1.append("\nOnline Users=" + pos1  );
